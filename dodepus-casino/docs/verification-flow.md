...
- `admin/verification`
  - **Поиск** по ID, имени, фамилии, e-mail, телефону, никнейму.
  - **4 секции-аккордеона** (по умолчанию свёрнуты): Requests, Partial, Rejected, Verified. У каждой есть счётчик и кнопка «Обновить».
  - **Карточка пользователя**: ID, отображаемое имя, прогресс X/4, бейджи модулей, количество документов.
  - **Модалка заявки**: просмотр данных, отметка модулей, комментарий, управление статусами, дозаполнение профиля, история и файлы.
...
- Блокировки полей зависят от статуса модуля:
  - 🔵/❌ — поле редактируемо.
  - 🟡/✅ — поле заблокировано до изменения статуса.
...
- Нажатие «Подтвердить» переводит модуль в 🟡, блокируя связанные поля в `profile/personal`.
...
- ❌ — карточка красная, показывается причина и дата, поля разблокируются, доступна повторная отправка (возврат в 🟡).
...
- Распределение карточек по секциям:
  ...
  - Нет 🟡, есть ≥1 ❌ — Rejected.
  - 4/4 ✅ — Verified.
- Карточка пользователя содержит ID, имя, прогресс, бейджи модулей, количество документов и подсказку «Нажмите, чтобы открыть заявку».
- Бейджи модулей: серые (просмотр), жёлтые/красные (активные действия), зелёные (только просмотр). Клик открывает модалку с подсветкой соответствующего блока.

## 9. Админ: модалка заявки

- Шапка: «ID → Модуль» и кнопка закрытия.
...
- Доступные действия:
  - **Подтвердить** — переводит отмеченные модули в ✅.
  - **Отказать** — переводит выбранные модули в ❌.
  - **Правка профиля** — администратор может дозаполнить поля профиля перед решением (все изменения попадают в историю).
  - **Сброс** — доступен в Verified; выбранные ✅→🔵, карточка перемещается в нужную секцию.

## 10. Сброс и маршрутизация

- Сброс доступен только для подтверждённых модулей в Verified.
- После сброса прогресс X/4 уменьшается, карточка переходит в Partial или, при мгновенном запуске новой заявки, в Requests.
- Перемещение между секциями всегда вычисляется по актуальным статусам модулей.

## 11. Параллельные запросы и приоритеты

- Клиент может держать несколько модулей в 🟡 одновременно — решения независимы.
- Если статусы смешанные, пока есть хотя бы один 🟡, карточка остаётся в Requests.
...
|-------------------|----------------------------------------|----------------|---------------------|-------------|
| 🔵 `idle`         | Клиент нажимает «Подтвердить»          | Клиент         | 🟡 `pending`        | Отправка заявки, поля блокируются. |
...
| 🟡 `pending`      | Админ отмечает часть модулей как готовые | Админ        | 🟡 `pending`        | Неподтверждённые модули остаются в ожидании. |
| ❌ `rejected`     | Клиент повторно отправляет заявку      | Клиент         | 🟡 `pending`        | Требуется исправление данных или новая загрузка. |
| ✅ `approved`     | Админ выполняет сброс выбранных модулей | Админ          | 🔵 `idle`           | История фиксирует событие «Сброс», карточка уходит в Partial/Requests. |
...
- **Заявка** — пакет модулей, отправленных клиентом одновременно.

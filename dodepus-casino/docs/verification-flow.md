...
- `admin/verification`
  - **Поиск** по ID, имени, фамилии, e-mail, телефону, никнейму; ввод обрабатывается с дебаунсом ~400 мс и синхронизируется с параметром `?q=`.
...
- ❌ — карточка красная, показывается причина и дата, поля разблокируются, доступна повторная отправка (возврат в 🟡 `in_review`).
...
- Бейджи модулей: серые (просмотр), жёлтые/красные (активные действия), зелёные (только просмотр). Клик открывает модалку и подсвечивает соответствующий модуль.
...
- Доступные действия работают с активным модулем:
  ...
  - **Сброс** — доступен в Verified; активный модуль возвращается в 🔵 `waiting`, карточка перемещается в нужную секцию.
...
| 🟡 `in_review`    | Админ отмечает часть модулей как готовые | Админ        | 🟡 `in_review`      | Неподтверждённые модули остаются в ожидании. |

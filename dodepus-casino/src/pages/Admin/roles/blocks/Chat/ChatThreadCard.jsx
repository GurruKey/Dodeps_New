...
export default function ChatThreadCard({ threadId, title, messagePlaceholder }) {
  const thread = chatThreads.find((candidate) => candidate.id === threadId) ?? chatThreads[0];
      ...
      <Card.Body as={Form} onSubmit={handleSend} className="d-flex flex-column gap-3">
        ...
        <Stack direction="horizontal" gap={2} className="align-items-start">
          <Form.Control
            as="textarea"
            rows={2}
            placeholder={messagePlaceholder ?? 'Введите сообщение…'}
            value={draftMessage}

...
export default function GenderDobBlock() {
  ...
  // Если в профиле gender не male/female, не выбираем ничего
  ...
  useEffect(() => { const next = parseDob(user?.dob); setD(next.d); setM(next.m); setY(next.y); }, [user?.dob]);
  ...
  // Сохранение общей кнопкой
  useEffect(() => {
    const onSave = () => {
      if (!changed) return;
      const patch = { dob: newDob };
      if (gender === 'male' || gender === 'female') patch.gender = gender; // если не выбрано — не трогаем
      updateProfile(patch);
    };
